...
export default function TagManager({ onClose }: TagManagerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto overscroll-contain bg-black/60 backdrop-blur-sm">
      {/* Scrollable overlay: centers the modal when it fits, but stays reachable
          when iOS's layout viewport differs from the visible (dynamic) viewport —
          otherwise the header clips behind the address bar and can't be reached. */}
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="w-full max-w-2xl bg-white dark:bg-gray-900 rounded-2xl flex flex-col max-h-[90dvh] shadow-2xl">
        {/* Header */}
